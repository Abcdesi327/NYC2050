...
function buildWater(site,rnd,S){
  ...
  const head=mul(vec(inward,1),S.basinLength);
  const mouth=[0,0];
  ...
  /* land the town could stand on at all: dry, and on its own side of the water */
  const townside=p=>!acrossTheWater(p);
...
function buildPlan(site,water,rnd,S){
  ...
  return {staple:staple, quayU:quayU, quayPts:quayPts, rays:kept, rings:rings,
    streets:streets, gates:gates, densityAt:densityAt};
}
...
/* ===================================================================================
   ...
   =================================================================================== */
const HOUSES={dwelling:1, poor:1.55, craft:0.62, merchant:0.5, staple:0.22,
...
function streetsFrom(mesh,rings,rays,plan,S,water){
  const out=[];
  const R2D=1/D2R;
  ...
  plan.streets.forEach(st=>emit(st.pts,false,{cls:st.cls,name:st.name}));
...
/* ===================================================================================
   ...
   =================================================================================== */
function buildWall(mesh,rings,rays,water,S){
  let wk=1;
  for(let k=1;k<rings.length-1;k++)
    if(Math.abs(rings[k]-S.wallR)<Math.abs(rings[wk]-S.wallR)) wk=k;
  ...
  rays.filter(r=>r.kind==="gate").forEach(r=>{
    ...
    if(pick&&bd<7) gates.push({
      name:r.name.replace(/^The /,"").replace(/ (Way|Road|Street)$/,"")+" Gate",
  ...
  return {ring:pts, towers:towers, gates:gates, ringIndex:wk,
    open:arc.length<ring.length};
}
...
/* ===================================================================================
   ...
   =================================================================================== */
function buildLandmarks(site,water,plan,wall,S){
...
function buildCity(site,spread,opts){
  const S=Object.assign({
    ...
    stapleR:spread*0.115, coreR:spread*0.42, wallR:spread*0.96, reach:spread*1.62,
  ...
  (function solveWall(){
    ...
    S.wallR=Math.max(plan.rings[3]||edge,Math.min(edge,S.reach*0.9));
  ...
  const wall=buildWall(mesh,plan.rings,plan.rays,water,S);
  const streets=streetsFrom(mesh,plan.rings,plan.rays,plan,S,water)
    .filter(s=>s.pts.length>1);
  const landmarks=buildLandmarks(site,water,plan,wall,S);
  ...
  return {
    site:site, S:S, water:water, plan:plan, wall:wall, streets:streets,
...
A.city={ siteOf:siteOf, generate:generate, rng:rng, DENSITY:DENSITY, USES:USES,
  ST_WIDTH:ST_WIDTH, FRONTAGE:FRONTAGE,
